...
from .successor_mask import successor_mask, invert_relation, offset_values_mask
...
class TypeCollection(object):
    def __init__(
        ...
        # empty blacklist:
        self.set_bad_node(set(), set())
        if language_path is not None:
            article_links = np.load(
                join(language_path, "trie_index2indices_values.npy")
            )
            article_links_counts = np.load(
                join(language_path, "trie_index2indices_counts.npy")
            )
            self._weighted_articles = np.bincount(
                article_links, weights=article_links_counts
            ).astype(np.int32)
            if len(self._weighted_articles) != len(self.ids):
                self._weighted_articles = np.concatenate(
                    [
                        ...
                        np.zeros(
                            len(self.ids) - len(self._weighted_articles), dtype=np.int32
                        ),
                    ]
                )
        else:
    ...
    def relation(self, name):
        ...
        if name not in self._relations:
            ...
            self._relations[name] = OffsetArray.load(
                join(self.path, "wikidata_%s" % (name,)), compress=True
            )
    ...
    def get_name(self, identifier):
        if identifier >= self.num_names_to_load and self._web_get_name:
            try:
                return (
                    get_name(self.ids[identifier]) + " (" + self.ids[identifier] + ")"
                )
    ...
    def describe_connection(self, source, destination, allowed_edges):
        ...
        found_path = self.is_member_with_path(source_index, allowed_edges, [dest_index])
    ...
    def is_member_with_path(
        ...
        for field in fields:
            ...
            for el in field_parents:
                if (
                    el in member_fields
                    and el not in self._bad_node
                    and (root, el) not in self._bad_node_pair
                ):
                    return True, path + [field, el]
            for el in field_parents:
                if (
                    el in visited
                    or el in self._bad_node
                    or (root, el) in self._bad_node_pair
                ):
                    continue
                visited.add(el)
                res = self.is_member_with_path(
                    el,
                    fields,
                    member_fields,
                    max_steps,
    ...
    def get_inverted_relation(self, relation_name):
        ...
        if relation_name not in self._inverted_relations:
            new_values_path = join(
                self.path, "wikidata_inverted_%s_values.npy" % (relation_name,)
            )
            new_offsets_path = join(
                self.path, "wikidata_inverted_%s_offsets.npy" % (relation_name,)
            )
            ...
            if not exists(new_values_path):
                ...
                if self.verbose:
                    print(
                        "inverting relation %r (%r)"
                        % (
                            relation_name,
                            self.wikidata_names2prop_names[relation_name],
                        )
                    )
                new_values, new_offsets = invert_relation(
                    relation.values, relation.offsets
                )
            ...
            if self.verbose:
                print(
                    "load inverted %r (%r)"
                    % (relation_name, self.wikidata_names2prop_names[relation_name])
                )
    ...
    def satisfy(self, relation_names, active_nodes, max_steps=None):
        assert len(relation_names) > 0, "relation_names cannot be empty."
        if (
            self.cache
            and isinstance(active_nodes, (list, tuple))
            and len(active_nodes) < 100
        ):
            satisfy_key = (
                tuple(sorted(relation_names)),
                tuple(sorted(active_nodes)),
                max_steps,
            )
            ...
            satisfy_key = None
        inverted_relations = [
            self.get_inverted_relation(relation_name)
    ...
    def print_top_class_members(self, truth_table, name="Other", topn=20):
        if self._weighted_articles is not None:
            ...
            print(
                "%s category, %d articles linked in wikipedia:"
    ...
    def class_report(self, relation_names, truth_table, name="Other", topn=20):
        ...
        self.print_top_class_members(is_fp, topn=topn, name=name + " (fixed points)")
        if self._weighted_articles is not None:
    ...
    def load_blacklist(self, path):
        ...
        for el in blacklist["bad_node"]:
            if el not in self.name2index:
                warnings.warn(
                    "Node %r under `bad_node` is not a known wikidata id." % (el)
                )
                continue
        ...
        for el, oel in blacklist["bad_node_pair"]:
            if el not in self.name2index:
                warnings.warn(
                    "Node %r under `bad_node_pair` is not a known wikidata id." % (el)
                )
                continue
            if oel not in self.name2index:
                warnings.warn(
                    "Node %r under `bad_node_pair` is not a known wikidata id." % (oel)
                )
                continue
